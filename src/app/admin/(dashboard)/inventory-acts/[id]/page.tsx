...
export const metadata = { title: 'Акт инвентаризации' };

interface Props {
  params: { id: string };
}

export default async function InventoryActDetailPage({ params }: Props) {
  const result = await getWarehouseDoc('inventory-acts', params.id);
  if (!result) notFound();
