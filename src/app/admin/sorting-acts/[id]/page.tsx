...
export const metadata = { title: 'Акт пересорта' };

interface Props {
  params: { id: string };
}

export default async function SortingActDetailPage({ params }: Props) {
  const result = await getWarehouseDoc('sorting-acts', params.id);
  if (!result) notFound();
